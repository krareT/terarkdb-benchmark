...
echo $TMPDIR
cp ../../terarkschema/dbmeta_wikipedia_index_string.json $dirname/dbmeta.json
echo "####Now, running terarkdb benchmark"
echo 3 > /proc/sys/vm/drop_caches
free -m
date
export TerarkDB_WrSegCacheSizeMB=500
../../db_wikipedia_terark_index_string --benchmarks=fillrandom --num=$record_num --sync_index=1 --db=$dirname --resource_data=$file
free -m
date
du -s -b $dirname

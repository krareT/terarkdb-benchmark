		...
		private:
			...
			void DoWrite(ThreadState* thread, bool seq) {
				...
				while(getline(ifs, str) && avg != 0) {
					...
					if (writen == 9) {
						recRow.cur_counter = str;
						writen++;
						continue;
					}
					if (writen == 10) {
						recRow.cur_is_redirect = str;
						writen++;
						continue;
					}
					if (writen == 11) {
						recRow.cur_minor_edit = str;
						writen++;
						continue;
					}
					if (writen == 12) {
				...
				printf("linenumber %lld, recordnumber %lld, avg %lld, offset %d, time %s\n",linenumber, recordnumber, copyavg, offset, asctime(timenow));
			}
